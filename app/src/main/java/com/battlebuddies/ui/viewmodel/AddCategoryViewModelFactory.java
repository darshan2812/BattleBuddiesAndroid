...
public class AddCategoryViewModelFactory extends ViewModelProvider.NewInstanceFactory {
    // COMPLETED (2) Add two member variables. One for the database and one for the taskId
    private final AppDatabase mDb;
    private final int mTaskId;

    // COMPLETED (3) Initialize the member variables in the constructor with the parameters received
    public AddCategoryViewModelFactory(AppDatabase database, int taskId) {
        mDb = database;
        mTaskId = taskId;
    }
    ...
    public <T extends ViewModel> T create(Class<T> modelClass) {
        ...
        return (T) new AddCategoryViewModel(mDb, mTaskId);
    }
}
